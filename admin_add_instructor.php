...
<head>
  ...
  <title>Administrator - Add Instructor</title>
...
<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $insdept = isset($_POST['insdept']) ? $_POST['insdept'] : "";
    switch ($ugend) {
        case 'Male':
            $ugend = 'M';
            break;
    ...
    $personId = getNewPersonId($conn); // Function to get a new PersonID
    $insertPersonStmt->bind_param("issss", $personId, $fname, $mname, $lname, $ugend);
    $insertPersonStmt->execute();
    $insertPersonStmt->close();

    // Use prepared statement to prevent SQL injection
    $insertInstructorStmt = $conn->prepare("INSERT INTO INSTRUCTOR (InstructorID, PassHash, PersonID, DeptNo) VALUES (?, ?, ?, ?)");
    $instructorId = isset($_POST['instructorId']) ? $_POST['instructorId'] : "";
    $password = "1234pass"; // The password you want to hash
$passHash = md5($password); // Convert the password to MD5 hash
$insertInstructorStmt->bind_param("sssi", $instructorId, $passHash, $personId, $insdept);
...
<form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>" class="admin-user-edit">
  ...
  <div class="user-person">
    <input type="text" name="instructorId" placeholder="INSTRUCTOR ID" class="edit-id rect-round-sm">
    <br>
    <br>
  ...
  <div class="label-dept"><span>DEPARTMENT:</span></div>
